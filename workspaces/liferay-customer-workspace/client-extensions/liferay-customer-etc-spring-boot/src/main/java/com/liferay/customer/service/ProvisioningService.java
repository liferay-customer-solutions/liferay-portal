...
public class ProvisioningService {
	...
	@Cacheable("licenseKeys")
	public List<LicenseKey> checkLicenseExpiration(String filter) {
		if (_log.isInfoEnabled()) {
			_log.info("Checking license expiration with filter: " + filter);
		}
		...
		try {
			...
			Page<LicenseKey> licenseKeyPage =
				licenseKeyResource.getAccountAccountKeyLicenseKeysPage(
					StringPool.BLANK, null, filter, Pagination.of(0, 100),
					null);
	...
	@CacheEvict(allEntries = true, value = "licenseKeys")
	@Scheduled(cron = "0 0 * * * *")
