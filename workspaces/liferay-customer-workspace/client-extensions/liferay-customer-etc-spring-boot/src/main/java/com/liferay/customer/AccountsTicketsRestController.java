...
public class AccountsTicketsRestController extends BaseRestController {
	...
	private ResponseEntity<String> _getJSMTickets(
		...
		try {
			...
			for (JiraIssue jiraIssue : jiraIssues) {
				jsonArray.put(_toJSONObject(jiraIssue));
			}
	...
	private JSONObject _toJSONObject(JiraIssue jiraIssue) {
		return new JSONObject(
		).put(
			"link", jiraIssue.getTicketURL()
		).put(
			"status", jiraIssue.getStatus()
		).put(
			"subject", jiraIssue.getSummary()
		).put(
			"ticketId", jiraIssue.getKey()
		);
	}
