...
const SubscriptionsOverview = ({koroneikiAccount, loading}) => {
	...
	const translatedSubscriptionGroups = useMemo(() => {
		...
		const GROUPS_TO_FILTER_OUT = ['Liferay Cloud'];

		const itemsToProcess = items.filter((group) => {
			return !GROUPS_TO_FILTER_OUT.includes(group.name);
		});

		return itemsToProcess.map((group) => {
			if (
