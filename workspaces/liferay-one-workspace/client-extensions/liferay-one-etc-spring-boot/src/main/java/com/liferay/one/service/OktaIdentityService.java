...
// See LPD-89439: syncContact and createContact formerly accepted a Contact DTO
// (from osb-provisioning). Signatures now accept primitives; callers must be
// updated accordingly.

import com.liferay.one.OktaProperties;
import com.liferay.one.constants.OktaConstants;
import com.liferay.one.model.OktaUser;
import com.liferay.one.pubsub.Message;
import com.liferay.one.pubsub.OktaPublisher;
...
public class OktaIdentityService {
	...
	protected void init() {
		_webClient = _webClientBuilder.baseUrl(
			"https://" + _oktaProperties.getHost()
		).defaultHeader(
			HttpHeaders.AUTHORIZATION, "SSWS " + _oktaProperties.getApiToken()
		).build();
	}
	...
	@Autowired
	private OktaProperties _oktaProperties;

	@Autowired
	private OktaPublisher _oktaPublisher;
