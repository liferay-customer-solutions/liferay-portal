...
const SEARCH_STORAGE_KEY = '@liferay-one/search';
...
const menu = document.querySelector('.liferay-one-nav-menu-container');
...
const searchContainer = document.querySelector('.liferay-one-search-container');
	...
	for (const product of items) {
		const itemHTML = `
				<li class="results-items-list w-100">
					<a class="align-items-center border-radius-medium d-flex flex-row mb-0 text-dark text-decoration-none w-100" href="/web/one/marketplace/p/${product.slug}?keyword=${searchValue}">
...
async function main() {
	...
	search.addEventListener('keydown', async (event) => {
		...
		if (key === 'Enter') {
			...
			window.location.href = `/web/one/marketplace/applications?${queryParam}=${searchValue}${
				state.categorySelected !== 'All Categories'
...
async function renderFeaturedSection({
	...
	const featuredSectionKey = `@liferay-one/featured/${specificationValue.toLowerCase()}`;
	...
	viewAllLink.href = `/web/one/marketplace/applications?our-selection=${specificationValue}`;
	...
	for (const product of featuredSectionProducts) {
		...
		productLink.href = `/web/one/marketplace/p/${product.slug}`;
