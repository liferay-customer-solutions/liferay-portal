...
import type {DeliveryProduct, ProductType} from '~/types/product';
...
export class DeliveryProductModel {
	...
	get specificationValues() {
		...
		for (const specificationKey in ProductSpecificationKey) {
			...
			(
				_specifications as Record<
					keyof typeof ProductSpecificationKey,
					string | undefined
				>
			)[_key] = this.getSpecificationValue(ProductSpecificationKey[_key]);
		}
	...
	public getPrice() {
		...
		if (priceModel.toLowerCase() === 'Free'.toLowerCase()) {
			return 'Free';
		}
