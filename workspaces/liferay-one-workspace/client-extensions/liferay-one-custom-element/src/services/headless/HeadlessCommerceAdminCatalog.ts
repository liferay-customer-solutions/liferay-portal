...
import type {Catalog, CommerceOption} from '~/types/commerce';
import type {
	Product,
	ProductCategories,
	ProductOption,
...
export default class HeadlessCommerceAdminCatalog {
	...
	static async createVirtualProduct({
		...
		catalogId: number;
		categories: Partial<ProductCategories>[];
		description: string;
		name: string;
	...
	static async getProductSpecifications(productId: string | number) {
		const response = await fetcher<APIResponse<ProductSpecification>>(
			`/o/headless-commerce-admin-catalog/v1.0/products/${productId}/productSpecifications`
		);
