...
def _check_chain(report, scope):
	...
	for entitlement in entitlements:
		...
		if (
			definition
			and product
			and definition_products.get(definition)
			and definition_products[definition] != product
		):
			report.error(
				"Entitlement definition is not on the ordered product",
				"entitlement %s uses definition %s from product %s but its order "
				"item %s sells product %s"
				% (
					erc,
					definition,
					definition_products[definition],
					order_item,
					product,
				),
			)
...
def _check_coverage(report, scope, skus):
	definitions = scope["definitions"]
	entitlements = scope["entitlements"]
	...
	definitions_by_product = defaultdict(list)

	for definition in definitions:
		if definition.get("active"):
			definitions_by_product[
				definition.get("r_commerceProductToEntitlementDefinition_CProductERC")
			].append(definition["externalReferenceCode"])
	...
	for order_item in order_items:
		...
		if definitions_by_product.get(product):
			report.error(
				"Order item grants no entitlement",
				"order item %s (%s) sells product %s, which defines %d entitlement "
				"definition(s), but grants nothing"
				% (
					erc,
					order_item["_file"],
					product,
					len(definitions_by_product[product]),
				),
			)
		else:
			report.warning(
				"Product defines no entitlements",
				"order item %s (%s) sells product %s, which has no entitlement "
				"definitions" % (erc, order_item["_file"], product),
			)
...
def main():
	...
	_check_coverage(report, locals(), skus)
