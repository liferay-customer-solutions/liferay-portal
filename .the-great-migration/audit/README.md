...
**The companion plan is in [`../plan/`](../plan/):**
- [`system-spec.md`](../plan/system-spec.md) — proposed design for the consolidated workspace (14 locked-in decisions, data model, business logic, APIs, phased migration).
- [`entitlement-rules-review.md`](../plan/entitlement-rules-review.md) — rule-by-rule translation plan for Koroneiki's 62 entitlement definitions.
...
_\* Zendesk and LCS are already retired at the business layer; the code paths in `osb-provisioning` that target them are vestigial. See `../plan/system-spec.md` D6 and D8._
...
Most of the audit's follow-up work has been done and now lives in [`../plan/`](../plan/). What remains:
...
2. ~~Extract and review all 62 entitlement rules~~ — **done.** See [`../plan/entitlement-rules-review.md`](../plan/entitlement-rules-review.md).
3. **Still open** — confirm the `Provisioning_*` license tables' ownership (overlaps with #1 above).
4. ~~Decide the unified Account/Subscription/Contact model~~ — **done.** Locked in as D1, D2, D3, D4 in [`../plan/system-spec.md`](../plan/system-spec.md).
5. Plan data migration for Koroneiki's 17M-row `AuditEntry` — resolved per D14: archive to flat file (JSON lines on GCS), don't migrate.
