...
Codebase: `/home/ry/repos/liferay-portal-ee/modules/dxp/apps/osb/osb-provisioning/`
...
2. **Support region mapping:** Hard-coded map "Liferay X" → Region (11 sales regions → 9 `Account.Region` enum values). E.g., Liferay US/Canada → UNITED_STATES, Liferay Spain → SPAIN (or HUNGARY for Greece/Cyprus/Italy). Default: GLOBAL.
3. Duplicate check: search Koroneiki for existing account with same name → warning flag.
4. New vs update: check Dossiera ExternalLink. If exists → update (add contacts, roles, purchases). Else → create new.
5. Create account:
...
6. Validation warnings: max developer count vs actual contacts, opportunity type vs account existence mismatch.
7. **Zendesk ticket** (if product family ≠ "P"):
...
8. **Email:** `SubscriptionSender` for Provisioning onboarding; Analytics Cloud welcome if product name contains "Liferay Analytics Cloud Subscription - Business" or "Enterprise".
...
2. **Error ticket loop risk** — if Zendesk is unavailable, error ticket creation fails, which then tries to create an error ticket for *that* failure. Unclear if `BaseMessageSubscriber` catches exceptions from `zendeskTicketWebService.createZendeskTicket()`.
3. **Hard-coded product-name matching** — Analytics Cloud detection uses string substring match ("Liferay Analytics Cloud Subscription - Business"/"Enterprise"). Fragile to product rename.
4. **Hard-coded numeric Zendesk custom-field IDs** — Zendesk config carries raw IDs. No validation that fields exist.
5. **Support region hard-coded mapping** — 11-entry table mapping sales regions. No admin UI.
6. **LCS sentinels hard-coded** — `4102444800000L`, `10000`, `7000`/`6200`. Opaque magic numbers.
7. **Product family substring check** — `product.familyKey()` values "E"/"P"/"S" hard-coded.
8. **`BaseWebService` fails on any 3xx** — not just 4xx/5xx; redirects may break.
9. **`DebugRabbitMQMVCActionCommand` production-visible** at `/admin/debug_rabbitmq` — consider access control.
10. **No tests on subscribers** — `DossieraCreateMessageSubscriber`, `ProductMessageSubscriber`, `ProductPurchaseMessageSubscriber` lack unit tests for the parsing + orchestration logic.
11. **No transactional boundaries** — message subscribers have no `@Transactional`. Orphaned records possible if contact creation succeeds but team assignment fails.
12. **Email template localization fallback** — if a locale variant is missing, falls back to unsuffixed English template. No translation workflow noted.
13. **ProductBundle arguably unused** — not referenced by `DossieraCreateMessageSubscriber` or core flows. Admin-managed only. Legacy?
14. **Account hierarchy hinted, not implemented** — `AncestorAccounts` in `AccountReader` suggests parent-child, but Dossiera message doesn't capture parent ref.
15. **`ProductPurchase.properties` is schemaless JSON** — "environment", "sizing", "productType" with no enum validation.
16. **No OpenAPI/contract tests vs Koroneiki** — schema changes in Koroneiki only surface at runtime.
17. **Stripe integration hinted in ExternalLink domain, not implemented** — future or dead?
18. **AuditEntry web service exists but no local persistence** — audit records presumably in Koroneiki; historical tracking unclear.
