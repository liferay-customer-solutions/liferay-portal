  ...
  "body": {
    ...
    "content": [
      { "type": "paragraph", "content": [
        ...
        { "type": "text", "text": "<fileName>", "marks": [{ "type": "link", "attrs": { "href": "<signedUrl>" } }] },
...
Mapping:
- Assets `name` ← `AccountEntry.name`
- Assets `key` ← `AccountEntry.koroneikiAccountCode`
- Assets attributes: `region`, `tier`, `status`, `deploymentCount`, `activeSubscriptionCount`
...
**Endpoint:** `POST /o/one/v1/jira/webhook` (see `../api.md §3.3`).
...
**Handler:**
1. Verify HMAC.
2. Look up `SupportTicket` by `webhookEvent.issue.key`. If not found, create a new row (lazy creation pattern, see `../objects/support.md`).
3. Update `statusCached`, `priorityCached`, `assigneeEmail`, `subject`, `lastSyncedAt=now`.
4. Ack with 200.
...
Atlassian caps:
- REST API: `~150 req / 5 min` per user token (tier-dependent).
- Assets API: `~100 req / 5 min`.

Clients use token-bucket rate limiters sized to 80% of the cap to leave headroom. Retries: exponential backoff on 429 + `Retry-After` header respected.

---

## Priority mapping

| Workspace priority (cached, internal) | Jira priority |
|---|---|
| Critical | Highest |
| High | High |
| Medium | Medium |
...
2. **Assets write-back throttle** — 5 min per account may still swamp the Assets API for 18K accounts during a bulk backfill. Consider batching or a slow-roll on initial sync.
3. **Webhook event delivery reliability** — Jira webhook delivery is best-effort. Confirm the 1h TTL on SupportTicket.lastSyncedAt is acceptable for the longest cache miss window.
4. **ADF versioning** — Atlassian evolves the ADF schema. Pin to version 1; revisit when Jira announces breaking changes.
