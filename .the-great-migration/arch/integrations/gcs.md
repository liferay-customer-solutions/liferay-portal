...
Both buckets use `{env}` in (`dev`, `staging`, `prod`).
...
Workspace:
1. Validates `fileName`, `fileSize` (≤ 200MB for publisher assets, ≤ 50MB for ticket attachments), MIME type allow-list.
2. Creates `TicketAttachment` (or `PublisherAssetAttachment`) row with `uploadStatus=Uploading` / `state=Draft`.
3. Generates a GCS resumable-upload session URL:
   ```
...
2. Compares reported MD5 against GCS object metadata. Mismatch → delete object, reject.
3. Flips `uploadStatus=Complete`; records `fileSize`, `md5Checksum`.
4. For `TicketAttachment`: leaves `state=Draft` until the user approves. Approval triggers Jira comment post (see `jira.md §2`).
5. For `PublisherAssetAttachment`: kicks off async processing (virus scan, Commerce catalog registration); flips `processed=true` on success.

### 4. Failed / abandoned uploads
...
2. Generates signed URL:
   ```
   https://storage.googleapis.com/{bucket}/{gcsObject}?X-Goog-...&Expires={now+900}
   ```
3. Returns `302 Location: {signedUrl}` (or JSON body `{ url, expiresAt }`, caller-preference).

Signed URLs expire in 15 min for ticket attachments, 60 min for publisher assets (larger files take longer).

**For Jira-comment-embedded links** (when posting ADF comments), the signed URL's expiry is 7 days. Jira users need long-lived access; the tradeoff is documented — signed URLs in Jira are effectively short-term bearer credentials.

---

## Delete flow

`DELETE /o/one/v1/ticket-attachments/{id}` flips `state=Trashed`. The actual GCS object deletion happens asynchronously via the `TicketAttachmentTrashDrain` scheduled task (see `../business-logic.md`).
...
Per-type caps live in `liferay-one-instance-settings` config so they're tunable without a release.
...
- **Virus scan** — publisher asset uploads flow through ClamAV (via Cloud Run service) before `processed=true`. Ticket attachments are not scanned (considered lower risk; reviewed by support agents).
...
2. **7-day Jira ADF link** — long expiry is a pragmatic compromise. Alternative is a workspace-side link redirector (`/t/<token>` → 302 to signed URL regenerated on demand). Adds ops burden; defer unless security review pushes back.
3. **ClamAV on ticket attachments** — currently out of scope (customer-workspace today doesn't scan). Add if security team requests.
4. **Lifecycle policy on publisher-assets bucket** — no auto-delete means storage grows. Consider 5-year retention after `PublisherAsset` withdrawal.
