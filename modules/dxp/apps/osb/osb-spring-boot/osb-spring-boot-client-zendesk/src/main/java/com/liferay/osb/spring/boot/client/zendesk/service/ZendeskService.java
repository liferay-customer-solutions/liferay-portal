...
public class ZendeskService {
	...
	public void updateZendeskTickets(List<ZendeskTicket> zendeskTickets)
		...
		int batchSize = 100;
		int batchTotal = zendeskTickets.size();
		...
		for (int i = 0; i < batchTotal; i += batchSize) {
			...
			for (int j = i; j < batchEnd; j++) {
				ZendeskTicket zendeskTicket = zendeskTickets.get(j);

				batchJSONArray.put(
					new JSONObject(
					).put(
						"custom_fields",
						_transformToCustomFieldsJSONArray(
							zendeskTicket.getCustomFields())
					).put(
						"id", zendeskTicket.getZendeskTicketId()
					).put(
						"tags",
						_transformToTagsJSONArray(zendeskTicket.getTags())
					));
			}
